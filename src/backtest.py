...
def _calibration(rows):
    ...
    for lo,hi in [(50,60),(60,70),(70,80),(80,90),(90,101)]:
        ...
        for direction,correct,fret,pred,actual,declared in rows:
            conf=declared
            if lo <= conf < hi: bucket.append((pred,actual))
...
def walk_forward(df: pd.DataFrame,timeframe: str,horizon: int=3,min_history: int=210,step: int=3)->BacktestResult:
    ...
    for i in range(min_history,end,step):
        ...
        actual_up=1 if future>0 else 0
    ...
    if not rows: return BacktestResult(timeframe,0,0,0,0,0,0)
    ...
    return BacktestResult(timeframe,n,round(acc,2),round(ba,2),round(be,2),round(avg,3),round(brier,4),ece,bins)
