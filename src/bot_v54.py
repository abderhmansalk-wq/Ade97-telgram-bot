...
def _fmt_status():
    ...
    out=["📒 V5.4 Dual-Regime Forward Paper",f"Engine: {'🟢 ON' if on else '⚪ OFF'}","BTC | 15m / 1h / 4h","RANGE: LONG RR2.0 | TREND: LONG/SHORT RR1.5 | score>=2"]
...
async def start(update,context):
    await update.message.reply_text(
        '🤖 Crypto Multi-Agent Analyst V5.4 جاهز.\n'
        '/paper50 on — تشغيل Dual-Regime Paper\n'
        '/paper50 status — نتائج RANGE/TREND لكل فريم\n'
        '/paper50 open — الصفقات المفتوحة والمسافة إلى TP/SL\n'
...
async def help_cmd(update,context):
    await update.message.reply_text(
        'V5.4 يراقب BTC على 15m و1h و4h.\n'
        'RANGE وTREND مساران منفصلان بإحصائيات مستقلة.\n'
        '/paper50 open — يعرض السعر الحالي، المسافة إلى TP/SL ونسبة التقدم.\n'
        '/paper50 on | off | status | open | trades | why\n'
...
async def _open_text():
    ...
    if not rows:
        return '📭 لا توجد صفقات Paper مفتوحة الآن.'
    out=[f'📂 V5.4 OPEN POSITIONS — {len(rows)}']
    for r in rows:
        p=r.get('market_price')
        if p is None:
            out.append(f"\n🟡 BTC {r['timeframe']} {r['regime']} {r['side']}\nEntry {r['entry']:.2f} | TP {r['tp']:.2f} | SL {r['sl']:.2f}\n⚠️ تعذر جلب السعر الحالي")
            continue
        progress=float(r.get('progress_pct') or 0.0)
        tp_pct=float(r.get('tp_distance_pct') or 0.0)
        sl_pct=float(r.get('sl_distance_pct') or 0.0)
        state='🟢 باتجاه TP' if progress>0 else ('🔴 باتجاه SL' if progress<0 else '⚪ عند الدخول')
        out.append(
            f"\n{state} — BTC {r['timeframe']} {r['regime']} {r['side']}\n"
            f"Entry: {r['entry']:.2f} | Now: {p:.2f}\n"
            f"TP: {r['tp']:.2f} | distance {tp_pct:.2f}%\n"
            f"SL: {r['sl']:.2f} | distance {sl_pct:.2f}%\n"
            f"Progress to TP: {progress:+.1f}% | RR {float(r['rr']):.1f} | Score {r['score']}"
        )
    out.append('\n⚠️ Paper فقط — ليست أوامر تداول حقيقية.')
    return '\n'.join(out)
...
async def paper50_cmd(update,context):
    ...
    if action=='on':
        ...
        await update.message.reply_text('🟢 V5.4 مفعّل: RANGE + TREND على 15m/1h/4h، Paper فقط.')
        try:
        ...
        return
    if action=='off':
        set_enabled(False,chat_id); await update.message.reply_text('⚪ تم إيقاف فتح إشارات V5.4. السجل محفوظ.'); return
    if action=='open':
        msg=await update.message.reply_text('📡 أفحص الصفقات المفتوحة والسعر الحالي...')
        try: await msg.edit_text((await _open_text())[:4000])
        except Exception as e: await msg.edit_text(f'❌ خطأ open: {e}')
        return
    if action=='trades':
        rows=recent_trades(16)
        if not rows: await update.message.reply_text('📭 لا توجد صفقات V5.4 مسجلة بعد.'); return
        out=['📜 آخر صفقات V5.4:']
        for r in rows:
